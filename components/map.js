...
export default class Map extends React.Component {
  ...
  mapDidLoad() {
    ...
    this.map.on('mousemove', (e) => {
      const layerIds = this.map.queryRenderedFeatures(e.point)
        .filter(layer => !!layer.properties.id)
        .map(layer => layer.properties.id);

      const areEqual = compareArrays(layerIds, this.props.highlightedChallengeIds);
      if (!areEqual) {
        this.props.setHighlighted(layerIds);
      }
  ...
  highlightDidChange({ ids }) {
    ...
    this.map.setPaintProperty('challenges-points-highlight', 'circle-opacity', getOpacity({ ids }));
    this.map.setPaintProperty('challenges-areas-highlight', 'fill-opacity', getOpacity({ ids, maxOpacity: 0.4 }));
    this.map.setPaintProperty('challenges-areas-outline-highlight', 'line-opacity', getOpacity({ ids }));
  ...
  componentDidUpdate(prevProps) {
    ...
    const areEqual = compareArrays(prevProps.highlightedChallengeIds, this.props.highlightedChallengeIds);
    if (!areEqual) {
